...
def check(docs, errors, today, cfg):
    ...
    # it did not.
    trackers = {d.epic: d for d in docs
    ...
    for epic in {d.epic for d in docs if d.epic and not d.archived}:
        if epic not in trackers:
            ...
            if live:
                problems.append(
                    f"epic `{epic}` has {len(live)} live file(s) but no tracker in trackers/")
    return problems


def render(docs, today, cfg):
    ...
    def table(rows):
        out.append(f"| File | Type | Epic | {label.title()} | Updated | Note |")
        out.append("|---|---|---|---|---|---|")
        for d in rows:
            ...
            out.append(f"| [`{d.name}`]({d.path}) | {d.type} | {d.epic or '-'} | "
